...
test("unused map fallback is excluded from critical service-worker precache",()=>{
  assert.match(sitesBuild,/excludedPrecacheAssetNames = new Set\(\["fzz-logo\.png", "map-fallback\.jpg"\]\)/);
...
test("unused map fallback is excluded from packaged Android assets",()=>{
  ...
  assert.match(androidBuild,/excludes: \["downloads\/\*\.apk", "assets\/fzz-logo\.png", "assets\/map-fallback\.jpg", "szz-logo\.png"\]/);
});
