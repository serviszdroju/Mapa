...
Current protocol mail/mobile decision: sending a protocol by e-mail should use the fixed subject "SZZ kontrola záložního zdroje" and the body should end with "S pozdravem", the current sender/technician name, and the full Servis záložních zdrojů s.r.o. company block. The "Předán protokol ke zpracování" checkbox in detail protocol history should visually match the neighboring action buttons. Opening "Vyplnit protokol" should use the full-page protocol form only on phone-sized layouts; tablet and desktop should keep the protocol inside the detail panel.
