...
Current approved visual direction: implement the generated unified mock direction with a calmer narrow left sidebar, top primary actions without a standalone "Mapa" row, icon-led compact action rows, compact search/filter controls, map-first center, and a denser right detail drawer with smaller cards and rows. Keep this as visual-only refinement; do not change Firebase data flows, protocols, photos, documents, or point detail functions.
