...
Current legacy status/detail parity decision: the left-panel SZZ logo should match the original `karolopejlo/Mapa` treatment: direct `szz-logo-display.png` source in the sidebar HTML, transparent background, contain sizing, and no placeholder alt text while waiting for JS. Stop Stav markers should render grey even when other timing flags exist. Detail tabs should be compact dark grey buttons with the active tab white, and the active source in "Zdroje na tomto místě" should use a lighter selected treatment. Main protocol history should show the "Zpracováno" checkbox only for records handed off for processing or already processed. Legacy `siteEdits` must remain a two-way bridge with the old `karolopejlo.github.io/Mapa` site for ordered controls, ordered repairs, Stop Stav, and detail data.
